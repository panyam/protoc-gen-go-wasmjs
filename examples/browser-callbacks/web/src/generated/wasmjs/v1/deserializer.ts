...
import { Wasmjs_v1Factory } from "./factory";
import { wasmjs_v1SchemaRegistry } from "./schemas";

// Shared factory instance to avoid creating new instances on every deserializer construction
const DEFAULT_FACTORY = new Wasmjs_v1Factory();

/**
 * Schema-aware deserializer for wasmjs.v1 package
 * Extends BaseDeserializer with package-specific configuration
 */
export class Wasmjs_v1Deserializer extends BaseDeserializer {
  constructor(
    schemaRegistry = wasmjs_v1SchemaRegistry,
  ...
  static from<T>(messageType: string, data: any) {
    const deserializer = new Wasmjs_v1Deserializer(); // Uses default factory and schema registry
