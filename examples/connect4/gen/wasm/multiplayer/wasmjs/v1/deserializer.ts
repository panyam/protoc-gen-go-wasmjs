...
import { WasmjsV1Factory } from "./factory";
import { WasmjsV1SchemaRegistry } from "./schemas";

// Shared factory instance to avoid creating new instances on every deserializer construction
const DEFAULT_FACTORY = new WasmjsV1Factory();
...
/**
 * Schema-aware deserializer for wasmjs.v1 package
 */
export class WasmjsV1Deserializer {
  constructor(
    private schemaRegistry: Record<string, MessageSchema> = WasmjsV1SchemaRegistry,
  ...
  static from<T>(messageType: string, data: any) {
    const deserializer = new WasmjsV1Deserializer(); // Uses default factory and schema registry
