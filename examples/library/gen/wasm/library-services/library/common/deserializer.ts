...
import { LibraryCommonFactory } from "./factory";
import { LibraryCommonSchemaRegistry } from "./schemas";

// Shared factory instance to avoid creating new instances on every deserializer construction
const DEFAULT_FACTORY = new LibraryCommonFactory();
...
/**
 * Schema-aware deserializer for library.common package
 */
export class LibraryCommonDeserializer {
  constructor(
    private schemaRegistry: Record<string, MessageSchema> = LibraryCommonSchemaRegistry,
  ...
  static from<T>(messageType: string, data: any) {
    const deserializer = new LibraryCommonDeserializer(); // Uses default factory and schema registry
