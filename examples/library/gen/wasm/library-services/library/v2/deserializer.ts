...
import { LibraryV2Factory } from "./factory";
import { LibraryV2SchemaRegistry } from "./schemas";

// Shared factory instance to avoid creating new instances on every deserializer construction
const DEFAULT_FACTORY = new LibraryV2Factory();
...
/**
 * Schema-aware deserializer for library.v2 package
 */
export class LibraryV2Deserializer {
  constructor(
    private schemaRegistry: Record<string, MessageSchema> = LibraryV2SchemaRegistry,
  ...
  static from<T>(messageType: string, data: any) {
    const deserializer = new LibraryV2Deserializer(); // Uses default factory and schema registry
