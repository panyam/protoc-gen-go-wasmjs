...
import { StreamingV1Factory } from "./factory";
import { StreamingV1SchemaRegistry } from "./schemas";

// Shared factory instance to avoid creating new instances on every deserializer construction
const DEFAULT_FACTORY = new StreamingV1Factory();
...
/**
 * Schema-aware deserializer for streaming.v1 package
 */
export class StreamingV1Deserializer {
  constructor(
    private schemaRegistry: Record<string, MessageSchema> = StreamingV1SchemaRegistry,
  ...
  static from<T>(messageType: string, data: any) {
    const deserializer = new StreamingV1Deserializer(); // Uses default factory and schema registry
