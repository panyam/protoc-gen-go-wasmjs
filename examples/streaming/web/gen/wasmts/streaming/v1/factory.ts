...
import { TickRequest as ConcreteTickRequest, TickResponse as ConcreteTickResponse } from "./models";
 ...
 */
export class StreamingV1Factory {


  /**
   * Enhanced factory method for TickRequest
  ...
  newTickRequest = (
    ...
    data?: any
  ): FactoryResult<TickRequestInterface> => {
    const out = new ConcreteTickRequest();
  ...
  /**
   * Enhanced factory method for TickResponse
   * @param parent Parent object containing this field
   * @param attributeName Field name in parent object
   * @param attributeKey Array index, map key, or union tag (for containers)
   * @param data Raw data to potentially populate from
  ...
  newTickResponse = (
    ...
    attributeKey?: string | number,
    data?: any
  ): FactoryResult<TickResponseInterface> => {
    const out = new ConcreteTickResponse();
  ...
  getFactoryMethod(messageType: string): ((parent?: any, attributeName?: string, attributeKey?: string | number, data?: any) => FactoryResult<any>) | undefined {
    ...
    const currentPackage = "streaming.v1";
