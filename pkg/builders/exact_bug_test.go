...
func TestExactBug(t *testing.T) {
	t.Run("DemonstrateBugInBuildServiceClientData", func(t *testing.T) {
		...
		config := &GenerationConfig{
			ModuleName: "example", // From buf.gen.yaml
		}

		// Simulate getModuleName logic
		correctModuleName := config.ModuleName // Should be "example"
		...
		if correctModuleName == "example" {
			t.Logf("✅ After fix, we should see 'Example' in generated files")
		}
	})
}
